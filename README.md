...
it can't just be read back later) — alongside a per-attempt correlation ID
(a UUID, staged the same way), so this attempt's `radius-verify.log` entry
and the helper's eventual `/check` decision can be matched up directly
instead of by MAC + rough timestamp, which gets ambiguous during retries.
`check-policy.sh` runs from `post-auth`
